...
import edu.utulsa.masters.opdyn.sim.Driver;
import edu.utulsa.masters.opdyn.sim.Independent;
...
/**
 * The main statistics-gathering class.
 */
public class Statistics {
    ...
    public void onSimulationBegin() {
        for(StatModule m: modules) m.hookSimulationBegin();
    }

    public void onSimulationEnd() {
        for(StatModule m: modules) m.hookSimulationEnd();
    }

    public void onTrialBegin(Simulator sim, double indpVal, int trial) {
        for(StatModule m: modules) m.hookTrialBegin(sim,indpVal,trial);
    }

    public void onTrialEnd(Simulator sim) {
        for(StatModule m: modules) m.hookTrialEnd(sim);
    }

    public void onRoundBegin(Simulator sim, int round) {
        for(StatModule m: modules) m.hookRoundBegin(sim, round);
    }

    public void onRoundEnd(Simulator sim, int round) {
        for(StatModule m: modules) m.hookRoundEnd(sim, round);
    }
