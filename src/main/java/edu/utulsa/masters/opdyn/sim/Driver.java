...
import edu.utulsa.masters.opdyn.stats.Statistics;
...
public class Driver {
    ...
    public void runTrialSet(double indpVal) {
        ...
        NetworkGenerator NG2 = (NetworkGenerator)netGen.updateValue(indp, indpVal);
    ...
    public void runTrial(NetworkGenerator ng, Interact i, int trialNum) {
        ...
        do {
            ...
            Debug.format(20, "Round %d %10.8f\n", round, sim.getTotalOpinionDifference());
            if(sim.getTotalOpinionDifference() <= OPD_THRESHOLD) pass++;
            else pass = 0;
            round++;
        } while(pass < 100);
    ...
    enum NetworkType { Random, Complete, BarabasiAlbert, WattsStrogatz, Group}
    enum InteractionType { BC,M1,M2 }
    public void loadConfig(File f) throws IOException {
        ...
        if(propIndp.equals("EPSILON")) indp = Independent.EPSILON;
        else if(propIndp.equals("DELTA")) indp = Independent.DELTA;
        else if(propIndp.equals("MU")) indp = Independent.MU;
        else if(propIndp.equals("DEGREE")) indp = Independent.DEGREE;
        else indp = Independent.NONE;
        ...
        String propOPDThreshold = p.getProperty("OpinionDifferenceThreshold","1E-4");
        ...
        if(propSeed != null) seed = Long.parseLong(propSeed);
        else seed = System.currentTimeMillis();
        rand = new Random(seed);
        TRACK_ACTIVITY = Boolean.parseBoolean(propTrackActivity);

        //Get network style
        String propNetworkType = p.getProperty("NetworkType","COMPLETE").toUpperCase();
        if(propNetworkType.equals("RANDOM")) {
            ...
            String propWSK = p.getProperty("WattsStrogatz_K","5");
