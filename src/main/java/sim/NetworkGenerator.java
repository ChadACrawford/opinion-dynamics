 ...
 */
public abstract class NetworkGenerator implements IndependentVariable {
    ...
    public static void smallworld(Network N, List<Agent> A, int K, double B) {
        int numOfAgents = A.size();

        if (K % 2 == 1) {
            System.out.println("K must be an even integer. Changing from " + K + " to " + (K - 1));
            K -= 1;
        }
        if (!((numOfAgents > K) && (K > Math.log(numOfAgents)) && (Math.log(numOfAgents) > 1))) {
            throw new IllegalArgumentException("Please check the relation between N and K: N > K, K > ln(N)");
        }

        for (int i = 0; i < numOfAgents; i++) {
            Agent a1 = A.get(i);
            for (int j = 0; j <= i; j++) {
                if (((Math.abs(i - j) % (numOfAgents - (K / 2))) <= (K / 2)) && i != j) {
                    Agent a2 = A.get(j);
                    N.addEdge(a1, a2);
                }
            }
            ...
            for (Agent b : an) {
                if (b.id > a.id) {
                    if ((rand.nextDouble() < B)) {
                        rn.add(b);
                        Agent c = CList.choose(rand, A);
                        while ((a == c) || N.isEdge(a, c)) {
                            c = CList.choose(rand, A);
                        }
                        N.addEdge(a, c);
                    }
                }
    ...
    public static class NGRandom extends NetworkGenerator {
        ...
        public IndependentVariable updateValue(Independent I, double V) {
